...
int main(int argc, char *argv[])
{
  ...
  while ((byte = fgetc(codefile)) != EOF) { //Make sure it's not an EOF before casting it is a char.
    ...
    if (prev) { //Only start parsing if we have 2 chars loaded already.
      if (comment) {
        switch(comment) {
          case 1: if (c == '\n') {comment = 3;} //Double slash comments are delimited by newlines.
          case 2: if (prev == '*' && c == '/') {comment=3;} //Multiline comments are delimited by the sequence of chars "*/"
          default: comment=0; //If we just ended a comment (case 3), skip over copying the next char (which will be part of the comment delimiter) but then get back to copying the file contents straight into the buffer.
        }
      } else if (prev == '/' && c == '/') {
        comment = 1;
      } else if (prev == '/' && c == '*') {
